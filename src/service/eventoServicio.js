...
const crearEvento = (async (parametros) => {
    ...
    const consulta = 'INSERT INTO evento (titulo, fecha, lugar, hora, imagen, descripcion, direccion, coordenadas, idOrganizador) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
    return await ejecutarConsulta(consulta, parametros);
});
...
const modificarEvento = (async (parametros) => {
    const consulta = 'UPDATE evento SET titulo = ?, fecha = ?, lugar = ?, hora = ?, imagen = ?, descripcion = ?, direccion = ?, coordenadas = ?, idOrganizador = ? WHERE id = ?';
    return await ejecutarConsulta(consulta, parametros);
});
